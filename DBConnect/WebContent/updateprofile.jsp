...
<html>
    ...
    <body>
		...
		<form method="post">
		<textarea style="height: 200px; width: 400px;" name="description"> <c:out value="${row.description}" /> </textarea>
		<br>
		...
		<c:if test="${pageContext.request.method=='POST'}">
			<c:catch var="exception">
				<sql:update dataSource="${firstname}" var="updatedTable">
    				UPDATE sp_details SET description = ? WHERE sp_email="<%= session.getAttribute("email") %>";
    				<sql:param value="${param.description}" />
				</sql:update>
				<c:if test="${updatedTable>=1}">
					<p id="response"> Thank you! Your profile has been updated.</p>
				</c:if>
			</c:catch>
